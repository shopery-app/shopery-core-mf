...
const ProductCard = memo(({ product, onAddToCart, isInCart, cartQuantity }) => {
  ...
  const handleAddClick = useCallback(
      (e) => { e.preventDefault(); onAddToCart(product); },
      [onAddToCart, product]
  );
      ...
      <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2">
        ...
        <div className="p-4">
          ...
          <div className="flex items-center justify-between">
            <div className="text-xl font-bold text-emerald-600">${product.currentPrice}</div>
            <button
            ...
            >
              {isInCart ? (
                  <><i className="fa-solid fa-check"></i>In Cart ({cartQuantity})</>
              ) : (
...
const ShopActions = memo(({ onAction }) => (
    <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-100">
      <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center">
        <i className="fa-solid fa-shopping-bag mr-2 text-emerald-600"></i>Shop Actions
      </h3>
      <div className="grid grid-cols-2 gap-3">
        ...
        >
          ...
          <span className="text-sm font-medium text-gray-700 group-hover:text-blue-700">Contact Shop</span>
        </button>
        <button
    ...
    <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-100">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-bold text-gray-800 flex items-center">
          <i className="fa-solid fa-star mr-2 text-emerald-600"></i>Featured Products
        </h3>
      ...
      {products && products.length > 0 ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-gray-500 text-sm mt-1">Check back later for amazing products!</p>
          </div>
      )}
    </div>
));
...
const ShopDetail = memo(() => {
  ...
  const fetchShopData = useCallback(async () => {
    ...
    try {
      ...
      if (error.response?.status === 404) setError("Shop not found.");
      else setError("Failed to load shop data. Please try again.");
    } finally {
      setLoading(false);
    }
  }, [shopId, navigate]);
  ...
  const handleBackToShops = useCallback(() => navigate("/shops"), [navigate]);
  const handleContactShop = useCallback(() => {}, []);

  const handleShareShop = useCallback(() => {
    if (navigator.share && shopData) {
      navigator.share({ title: shopData.shopName, text: shopData.description, url: window.location.href }).catch(console.error);
    } else {
  ...
  const currentUser = profile ? { id: profile.id, firstName: profile.firstName } : null;
  const isOwnShop = currentUser?.id && shopData?.sellerId && currentUser.id === shopData.sellerId;
    ...
    return (
        <Suspense fallback={<LoadingSpinner />}>
          ...
          <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center">
            <div className="text-center"><LoadingSpinner /><p className="mt-6 text-gray-600 font-medium">Loading shop details...</p></div>
          </div>
          <Footer />
        </Suspense>
    );
        ...
        <Suspense fallback={<LoadingSpinner />}>
          ...
          <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center">
            <div className="text-center max-w-md mx-auto p-6">
              <div className="text-red-500 mb-6"><i className="fa-solid fa-store-slash text-6xl"></i></div>
              <h3 className="text-2xl font-bold text-gray-800 mb-4">Shop Not Found</h3>
              <p className="text-gray-600 mb-6">{error || "The shop you're looking for doesn't exist or has been removed."}</p>
              <button onClick={handleBackToShops} className="bg-gradient-to-r from-emerald-600 to-emerald-700 text-white px-6 py-3 rounded-xl font-semibold hover:from-emerald-700 hover:to-emerald-800 transition-all duration-300">
                <i className="fa-solid fa-arrow-left mr-2"></i>Back to Shops
              </button>
            </div>
          </div>
          <Footer />
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100">
          <div className="relative bg-gradient-to-r from-emerald-600 to-emerald-700 text-white py-14 shadow-lg">
            ...
            <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row items-center justify-between">
              <div className="flex items-center gap-6">
                ...
                <div>
                  <h1 className="text-4xl md:text-5xl font-extrabold drop-shadow-lg">{shopData.shopName}</h1>
                  <p className="text-emerald-100 mt-2 text-lg font-medium">Welcome to our shop</p>
                </div>
              </div>
              <div className="flex gap-3 mt-6 md:mt-0">
                <button onClick={handleContactShop} className="bg-white text-emerald-600 px-6 py-3 rounded-xl font-semibold hover:bg-emerald-50 transition-all duration-300 flex items-center gap-2 shadow">
                  <i className="fa-solid fa-envelope"></i>Contact Shop
                </button>
                <button onClick={handleShareShop} className="bg-emerald-500 text-white px-6 py-3 rounded-xl font-semibold hover:bg-emerald-400 transition-all duration-300 flex items-center gap-2 shadow">
          ...
          <div className="max-w-7xl mx-auto px-4 py-10">
              ...
              <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
                <div className="flex items-center justify-between mb-8">
                  <h3 className="text-xl font-bold text-gray-800 flex items-center gap-2">
                    <i className="fa-solid fa-box text-emerald-600"></i>Browse Products
                  </h3>
                ...
                {productsLoading ? (
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="text-gray-500 text-base mt-2">Try searching with a different keyword!</p>
                    </div>
                ) : (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
              <div className="lg:col-span-1"><ShopActions onAction={handleShopAction} /></div>
              <div className="lg:col-span-2"><FeaturedProducts products={products} /></div>
            </div>

            <div className="mt-8">
              <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
                <h3 className="text-xl font-bold text-gray-800 mb-6 flex items-center gap-2">
                  <i className="fa-solid fa-info-circle text-emerald-600"></i>About This Shop
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div>
                    <p className="text-gray-600 text-sm font-medium">Shop Name</p>
                    <p className="text-gray-800 font-semibold text-lg">{shopData.shopName}</p>
                  </div>
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex text-yellow-400">
                        {[1, 2, 3, 4, 5].map((star) => (
                            <i key={star} className="fa-solid fa-star text-base"></i>
                        ))}
                      </div>
                      <span className="text-gray-800 font-semibold">{shopData.rating || "4.8"}</span>
                  ...
                  <div>
                    ...
                    <p className="text-gray-800 font-semibold">
                      {shopData.createdAt ? new Date(shopData.createdAt).toLocaleDateString() : "N/A"}
                    </p>
                  </div>
                  <div>
                    <p className="text-gray-600 text-sm font-medium">Total Products</p>
                    <p className="text-gray-800 font-semibold">{products.length} products</p>
                  </div>
                  <div className="md:col-span-2">
                    <p className="text-gray-600 text-sm font-medium">Description</p>
                    <p className="text-gray-800 leading-relaxed mt-1 text-base">{shopData.description}</p>
                  </div>
                </div>
              </div>
            </div>
