...
// FIX: checkoutAPI only creates a Stripe session — it does NOT return orders.
// Orders are created by the backend webhook AFTER Stripe confirms payment.
// So on success we just get { sessionId, checkoutUrl } and redirect.
export const checkoutAPI = createAsyncThunk("cart/checkout",
    async (_, { rejectWithValue }) => {
        try {
            ...
            // Returns StripeCheckoutResponseDto: { sessionId, checkoutUrl }
            return data?.data;
        } catch (err) {
...
const initialState = {
    ...
    checkoutError: null,
    // Stores the Stripe session info after checkout is initiated
    stripeSession: null,
    loading: false,
    error: null,
};
...
const cartSlice = createSlice({
    ...
    reducers: {
        ...
        // FIX: allow clearing the stripe session (e.g. after redirect back)
        clearStripeSession: (state) => { state.stripeSession = null; },
    },
    extraReducers: (builder) => {
        builder
            ...
            // FIX: fulfilled means we got a Stripe URL back — store it, reset loading
            // The cart is NOT cleared here; it's cleared by the webhook after payment succeeds
            .addCase(checkoutAPI.fulfilled, (state, action) => {
                state.checkoutLoading = false;
                state.stripeSession = action.payload; // { sessionId, checkoutUrl }
            })
            // FIX: always reset checkoutLoading on rejection
